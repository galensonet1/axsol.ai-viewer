...
const LayerSelector = ({
  ...
  const handleLayerChange = (event) => {
    ...
    // Track first use of specific layers
    if (enabled) {
      if (layerType === 'plan') {
        trackFeatureFirstUse('plan_layer', { project_id: projectId });
      } else if (layerType === 'fotos360') {
        trackFeatureFirstUse('photos_360_layer', { project_id: projectId });
      } else if (layerType === 'realidad3D') {
        trackFeatureFirstUse('reality_3d_layer', { project_id: projectId });
      }
    }
                  ...
                  <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <DateCalendar
                      ...
                      onChange={(value) => {
                        ...
                        const newDate = value.startOf('day').format('YYYY-MM-DD');
                        
                        // 📊 ANALYTICS: Track capture date changed
