...
// Componente que detecta cuando el viewer de Cesium está listo via useCesium
const ViewerReadyDetector = ({ onViewerReady, viewerLoadedTrackedRef, projectId }) => {
  const { viewer } = useCesium();
  
  console.log('🔵 [ViewerReadyDetector] RENDER - viewer disponible:', !!viewer, 'tracked:', viewerLoadedTrackedRef.current);
  
  useEffect(() => {
    console.log('🔵 [ViewerReadyDetector] useEffect ejecutado - viewer:', !!viewer, 'tracked:', viewerLoadedTrackedRef.current);
    
    if (viewer && !viewerLoadedTrackedRef.current) {
      console.log('🎯 [ViewerReadyDetector] Viewer detectado via useCesium!');
      onViewerReady(viewer);
    }
  }, [viewer, onViewerReady, viewerLoadedTrackedRef]);
...
const ProjectVisualizer = () => {
  ...
  const viewerLoadedTrackedRef = useRef(false); // 📊 ANALYTICS: Track viewer_loaded solo una vez
  ...
  // Deep link functionality (share view temporalmente deshabilitado)
  const { getViewFromURL } = useDeepLink();
  ...
  useEffect(() => {
    ...
    // console.log('🔍 [PROJECT DEBUG] tipo:', typeof projectData?.weekly_construction_plan);
    // console.log('🔍 [PROJECT DEBUG] es null?:', projectData?.weekly_construction_plan === null);
    // console.log('🔍 [PROJECT DEBUG] es undefined?:', projectData?.weekly_construction_plan === undefined);
    // console.log('🔍 [PROJECT DEBUG] boolean:', Boolean(projectData?.weekly_construction_plan));
  }, [projectData]);
  ...
  const hasActivityPlan = useMemo(() => {
    ...
    // console.log('📋 [PLAN DEBUG] Campo existe?:', 'weekly_construction_plan' in (projectData || {}));
    return result;
  ...
  const planUrl = useMemo(() => {
    try {
      ...
      if (raw && typeof raw === 'string') {
        if (/^https?:\/\//i.test(raw)) {
          // console.log('🔗 [PLAN DEBUG] URL absoluta detectada:', raw);
          return raw;
        }
        ...
        // console.log('⚠️ [PLAN DEBUG] raw NO es string válido, tipo:', typeof raw);
      }
    } catch (error) {
      console.error('❌ [PLAN DEBUG] Error en planUrl:', error);
    }
    // console.log('⚠️ [PLAN DEBUG] planUrl retornando NULL');
    return null;
  ...
  const handleViewerReady = useCallback((viewer) => {
    console.log('🎬🎬🎬 [handleViewerReady] ========== EJECUTADO ========== 🎬🎬🎬');
    console.log('🎬 [handleViewerReady] Viewer:', viewer);
    console.log('🎬 [handleViewerReady] Viewer tiene scene:', !!viewer?.scene);
    console.log('🎬 [handleViewerReady] PostHog disponible:', !!window.posthog);
    console.log('🎬 [handleViewerReady] window.posthog object:', window.posthog);
    console.log('🎬 [handleViewerReady] trackEvent disponible:', typeof trackEvent);
    console.log('🎬 [handleViewerReady] projectId:', projectId);
    
    setCesiumViewer(viewer);
    
    // 📊 ANALYTICS: Track viewer_loaded
    if (!viewerLoadedTrackedRef.current) {
      console.log('🎬✅ [Analytics] handleViewerReady - trackeando viewer_loaded');
      try {
        trackEvent('viewer_loaded', {
          project_id: projectId,
          cesium_version: Cesium.VERSION || 'unknown'
        });
        viewerLoadedTrackedRef.current = true;
        console.log('✅ [Analytics] viewer_loaded trackeado exitosamente');
      } catch (error) {
        console.error('❌ [Analytics] Error trackeando viewer_loaded:', error);
      }
    }
  ...
  }, [projectId]); // Dependencies: projectId, trackEvent, trackError, setCesiumViewer

  // 📊 ANALYTICS: viewer_loaded se trackea via ViewerReadyDetector cuando el viewer está listo
  ...
  // Funciones para Deep Links
  const applyViewFromURL = useCallback((viewState) => {
    if (!cesiumViewer || !viewState) return;
    
    try {
      // console.log('[ProjectVisualizer] Aplicando vista desde URL:', viewState);
      
      const { camera, date, layers, ifcHeightOffset: urlIfcOffset } = viewState;
      
      // Aplicar posición de cámara con animación
      ...
      // Aplicar fecha si es válida
      if (date && date !== selectedCaptureDate) {
        setSelectedCaptureDate(date);
      }
      
      // Aplicar capas activas
      if (Array.isArray(layers)) {
        setLayerVisibility(prev => ({
          ...prev,
          proyecto3D: layers.includes('ifc'),
          fotos: layers.includes('photos'),
          fotos360: layers.includes('photos360'),
          plan: layers.includes('activities')
        }));
      }
      
      // Aplicar offset de IFC si está presente
      if (typeof urlIfcOffset === 'number' && urlIfcOffset !== ifcHeightOffset) {
        setIfcHeightOffset(urlIfcOffset);
      }
      
    } catch (error) {
      console.error('[ProjectVisualizer] Error aplicando vista desde URL:', error);
    }
  }, [cesiumViewer, selectedCaptureDate, ifcHeightOffset]);

  // Cargar vista desde URL al inicializar
  useEffect(() => {
    console.log('[ProjectVisualizer] Checking for view in URL, cesiumViewer:', !!cesiumViewer);
    if (!cesiumViewer) return;
    
    const viewFromURL = getViewFromURL();
    console.log('[ProjectVisualizer] View from URL:', viewFromURL);
    if (viewFromURL) {
      console.log('[ProjectVisualizer] Applying view from URL in 1 second...');
      // Esperar un poco para que el visor esté completamente inicializado
      setTimeout(() => {
        applyViewFromURL(viewFromURL);
      }, 1000);
    }
  }, [cesiumViewer, getViewFromURL, applyViewFromURL]);
  ...
  console.log('🚨 cesiumViewer object:', cesiumViewer); // Ver el objeto completo
  console.log('🚨 handleViewerReady function:', typeof handleViewerReady);
  console.log('🚨 projectId:', projectId);
  console.log('🚨 terrainProvider:', !!terrainProvider);
  console.log('🚨 window.posthog disponible:', !!window.posthog);
...
{displayClockLabel && <Box className="clock-label">{displayClockLabel}</Box>}
      ...
      >
        <ViewerReadyDetector 
          ...
          viewerLoadedTrackedRef={viewerLoadedTrackedRef}
          projectId={projectId}
        />
...
const CzmlLayer = ({ data, visible, layerKey, splitDirection = null }) => {
  ...
  // Debug para Plan de Actividades
  useEffect(() => {
    if (layerKey === 'plan') {
      console.log(`🎬 [CzmlLayer-${layerKey}] Props recibidos:`, { data, visible, layerKey });
      console.log(`🎬 [CzmlLayer-${layerKey}] data es:`, typeof data, data);
    }
  }, [data, visible, layerKey]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      if (!data) {
        if (layerKey === 'plan') {
          console.log(`⚠️ [CzmlLayer-${layerKey}] NO HAY DATA, no se cargará nada`);
        }
      ...
      if (layerKey === 'plan') {
        console.log(`🚀 [CzmlLayer-${layerKey}] Iniciando carga con data:`, data);
      }
      ...
      try {
        if (layerKey === 'plan') {
          console.log(`📥 [CzmlLayer-${layerKey}] Llamando CesiumCzmlDataSource.load...`);
        }
        const ds = await CesiumCzmlDataSource.load(data, {
          sourceUri: `${layerKey || 'czml'}.json`,
        });
        if (layerKey === 'plan') {
          console.log(`✅ [CzmlLayer-${layerKey}] DataSource cargado exitosamente!`);
          console.log(`✅ [CzmlLayer-${layerKey}] Entidades cargadas:`, ds.entities.values.length);
        }
